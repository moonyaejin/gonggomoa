...
- **작업(요청) 단위로 feature 브랜치를 만들어 그 위에서 작업한다.** `main`에 직접 커밋하지 않는다. 브랜치 생성 자체는 Claude가 바로 한다(되돌리기 쉬운 로컬 작업이므로).
- **커밋·푸시·PR 생성은 항상 제안만 한다.** 작업이 끝나면 커밋 메시지 한 줄과 `git push -u origin <브랜치명>` 명령, 그리고 PR 제목·본문을 함께 제안하고, 실제 `git commit`·`git push`·`gh pr create` 실행은 사용자가 직접 한다. Claude가 먼저 커밋·푸시·PR 생성을 실행하지 않는다.

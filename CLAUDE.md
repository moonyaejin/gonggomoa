...
- 한 번에 한 기능. 여러 기능을 묶어서 커밋하지 않는다. 또한 기능별로 브랜치를 나누어 작업한다.
- **코드가 바뀌는 작업은 작업(요청) 단위로 feature 브랜치 + PR.** `main`에 직접 커밋하지 않는다. 브랜치 생성 자체는 Claude가 바로 한다(되돌리기 쉬운 로컬 작업이므로).
- **문서·설정만 바뀌는 작업(`docs/`, ADR, `.gitignore`, `CLAUDE.md` 등 코드가 아닌 파일)은 브랜치·PR 없이 `main`에 바로 커밋한다.** 절차 비용 대비 리뷰로 얻는 가치가 없기 때문이다. 코드 변경이 하나라도 섞이면 코드 기준(브랜치+PR)을 따른다.
- **커밋·푸시·PR 생성은 항상 제안만 한다.** 작업이 끝나면 커밋 메시지 한 줄과 `git push -u origin <브랜치명 또는 main>` 명령(코드 변경이면 PR 제목·본문도 함께)을 제안하고, 실제 `git commit`·`git push`·`gh pr create` 실행은 사용자가 직접 한다. Claude가 먼저 커밋·푸시·PR 생성을 실행하지 않는다.
...
| `docs/08-external-api.md` | 재정경제부 채용정보 API 실제 응답 구조     |

...
**호출량 절감의 핵심은 2단 판정이다** — `/list`로 받은 `scrnprcdrMthdExpln`을 먼저 읽어 필기전형이 없는 공고를 걸러낸 뒤, 남은 공고에만 `/detail`을 호출한다. 표본상 필기 시행 공고는 전체의 10% 내외이므로 `/detail` 호출량이 1/10로 줄고, 첨부파일 파싱 대상도 같은 비율로 감소한다.
